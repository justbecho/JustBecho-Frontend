...
import AuthModal from '@/components/ui/AuthModal' // ✅ Auth Modal Import
...
export default function ProductPage() {
  ...
  useEffect(() => {
    const checkLoginStatus = () => {
      if (typeof window !== 'undefined') {
        ...
        // ✅ Agar logged in hai toh cart items fetch karo
        if (token) {
  ...
  // ✅ Generic authentication check function
  ...
  const handleAddToCart = async () => {
    ...
    if (!requireAuth('addToCart')) return
    ...
    try {
      ...
      if (data.success) {
        ...
        setCartItems(prev => [...prev, { product, ...cartData }])
      } else {
  ...
  const handleBuyNow = async () => {
    ...
    // ✅ Agar product already cart mein hai, direct cart page pe redirect
    if (isProductInCart()) {
      router.push('/cart')
      return
    }
    
    // ✅ Agar product cart mein nahi hai, pehle add karein phir redirect
    try {
      setAddingToCart(true)
      const token = localStorage.getItem('token')
                ...
                <div className="space-y-3 pt-4 border-t border-gray-200">
                  ...
                  >
                    {addingToCart ? 'PROCESSING...' : (
                      isProductInCart() ? 'GO TO CART' : 'BUY NOW'
                    )}
                  </button>
                </div>
              </div>
